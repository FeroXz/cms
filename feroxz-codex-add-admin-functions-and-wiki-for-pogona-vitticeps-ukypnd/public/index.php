            ...
            if ($rawResults) {
                ...
                foreach ($rawResults['combined'] as $entry) {
                    ...
                    foreach ($entry['states'] as $geneId => $stateKey) {
                        $geneId = (int)$geneId;
                        if (!isset($geneLookup[$geneId])) {
                            continue;
                        }
                        $gene = $geneLookup[$geneId];
                        $label = $entry['labels'][$geneId] ?? gene_state_label($gene, $stateKey);
                        $states[] = [
                            'gene' => $gene['name'],
                            'state' => $stateKey,
                            'label' => $label,
                            'is_visual' => array_key_exists($geneId, $entry['visual_traits']),
                            'is_carrier' => array_key_exists($geneId, $entry['carrier_traits']),
                        ];
                    }

                    $results[] = [
                        'label' => trim((string)($entry['phenotype'] ?? '')) ?: 'Wildtyp',
