...
function create_genetic_gene(PDO $pdo, array $data): int
{
    ...
    $stmt = $pdo->prepare('INSERT INTO genetic_genes(species_id, name, slug, shorthand, inheritance_mode, description, normal_label, heterozygous_label, homozygous_label, originator, origin_url, image_path, is_reference, display_order) VALUES (:species_id, :name, :slug, :shorthand, :inheritance_mode, :description, :normal_label, :heterozygous_label, :homozygous_label, :originator, :origin_url, :image_path, :is_reference, :display_order)');
    $stmt->execute([
        ...
        'originator' => trim($data['originator'] ?? '') ?: null,
        'origin_url' => trim($data['origin_url'] ?? '') ?: null,
...
function update_genetic_gene(PDO $pdo, int $id, array $data): void
{
    ...
    $stmt = $pdo->prepare('UPDATE genetic_genes SET name = :name, slug = :slug, shorthand = :shorthand, inheritance_mode = :inheritance_mode, description = :description, normal_label = :normal_label, heterozygous_label = :heterozygous_label, homozygous_label = :homozygous_label, originator = :originator, origin_url = :origin_url, image_path = :image_path, is_reference = :is_reference, display_order = :display_order, updated_at = CURRENT_TIMESTAMP WHERE id = :id');
    $stmt->execute([
        'name' => $name,
        'slug' => $slug,
        'shorthand' => trim($data['shorthand'] ?? '') ?: null,
...
function synchronize_gene_samples(PDO $pdo, int $speciesId, array $samples): void
{
    foreach ($samples as $sample) {
        ...
        if ($existing) {
            $fields = ['name', 'shorthand', 'inheritance_mode', 'description', 'normal_label', 'heterozygous_label', 'homozygous_label', 'originator', 'origin_url', 'image_path', 'is_reference', 'display_order'];
            $needsUpdate = false;
            ...
            if ($needsUpdate) {
                update_genetic_gene($pdo, (int)$existing['id'], $payload);
            }
        } else {
...
function apply_gene_media_defaults(array $entries, string $originator, string $baseUrl, array $imagePool): array
{
    $pool = !empty($imagePool) ? array_values($imagePool) : ['https://images.unsplash.com/photo-1610970878458-1c0d565c4ce6?auto=format&fit=crop&w=1200&q=80'];
    $poolCount = count($pool);

    return array_map(static function (array $entry) use ($originator, $baseUrl, $pool, $poolCount) {
        $slug = $entry['slug'] ?? slugify($entry['name'] ?? bin2hex(random_bytes(4)));
        if (empty($entry['originator'])) {
            $entry['originator'] = $originator;
        }
        if (empty($entry['origin_url'])) {
            $entry['origin_url'] = rtrim($baseUrl, '/') . '/' . $slug;
        }
        if (empty($entry['image_path'])) {
            $entry['image_path'] = $pool[$poolCount > 0 ? crc32($slug) % $poolCount : 0];
        }
        return $entry;
    }, $entries);
}
...
function capitalize_label(string $label): string
{
    if ($label === '') {
        return $label;
    }
    ...
    return strtoupper($label[0]) . substr($label, 1);
}
...
function calculate_genetic_outcomes(array $genes, array $parentOneSelections, array $parentTwoSelections): ?array
{
    ...
    foreach ($combinedResults as $entry) {
        $visualKeyParts = [];
        foreach ($entry['visual_traits'] as $geneId => $label) {
            $stateKey = $entry['states'][$geneId] ?? 'normal';
            $visualKeyParts[] = $geneId . ':' . $stateKey;
        }
        sort($visualKeyParts);
        $groupKey = implode('|', $visualKeyParts);
        if ($groupKey === '') {
            $groupKey = 'none';
        }
        ...
        $grouped[$groupKey]['probability'] += $entry['probability'];
        $grouped[$groupKey]['entries'][] = $entry;
    }
    ...
    foreach ($grouped as $group) {
        ...
        $visualLabels = array_values($group['visual_traits']);
        if (!empty($visualLabels)) {
            $labelParts[] = implode(', ', $visualLabels);
        }
        ...
        foreach ($geneResults as $geneId => $geneResult) {
            ...
            foreach ($group['entries'] as $entry) {
                $stateKey = $entry['states'][$geneId] ?? 'normal';
                if (!isset($stateTotals[$stateKey])) {
                    $stateTotals[$stateKey] = 0.0;
                }
                $stateTotals[$stateKey] += $entry['probability'];
            }

            if (isset($group['visual_traits'][$geneId])) {
                $stateKey = $group['entries'][0]['states'][$geneId] ?? 'normal';
            ...
            $heteroProbability = $stateTotals['heterozygous'] ?? 0.0;
            if ($heteroProbability <= 0) {
                continue;
            }

            $heteroLabel = gene_state_label($gene, 'heterozygous');
            if ($heteroLabel === '') {
                $heteroLabel = 'het ' . $gene['name'];
            }
            $heteroLabel = capitalize_label($heteroLabel);

            if ($heteroProbability >= $groupProbability - 1e-8) {
                $label = '100% ' . $heteroLabel;
                $states[] = [
                    'gene' => $gene['name'],
                    'state' => 'carrier',
                    'label' => $label,
            ...
            $ratio = $heteroProbability / $groupProbability;
            $percent = (int)floor($ratio * 100 + 0.0001);
            if ($percent < 1) {
                $percent = 1;
            }

            $label = $percent . '% ' . $heteroLabel;
            $states[] = [
                'gene' => $gene['name'],
                'state' => 'possible',
                'label' => $label,
        ...
        $finalResults[] = [
            'label' => implode(' • ', array_values(array_unique($labelParts))),
            'probability' => $groupProbability,
            'states' => $states,
        ];
    }

    usort($finalResults, static function ($a, $b) {
        return $b['probability'] <=> $a['probability'];
    });

    return [
        'genes' => $geneResults,
        'combined' => $finalResults,
    ];
}
...
function ensure_default_genetics(PDO $pdo): void
{
    ...
    $baseGenes = [
        ...
        [
            ...
            'inheritance_mode' => 'recessive',
            'description' => 'Dunkle Schweizer Linie mit sattem Schokoladenton und klarer Kopfzeichnung. Häufig als Grundlage für High-Contrast-Kombinationen genutzt.',
            'normal_label' => 'Normal',
        ...
        [
            ...
            'slug' => 'shadow',
            'inheritance_mode' => 'recessive',
            'description' => 'Shadow reduziert das Melanin nicht vollständig, sorgt aber für kontrastreiche Seitenspots und einen rauchigen Kopfbereich.',
            'normal_label' => 'Normal',
            'heterozygous_label' => 'het Shadow',
        ...
        [
            ...
            'slug' => 'sunburst',
            'inheritance_mode' => 'dominant',
            'description' => 'Selektive Linie mit intensiv gelb-orangen Seiten und verstärktem Pattern-Reducing-Effekt.',
            'normal_label' => 'Normal',
            'heterozygous_label' => 'Sunburst',
            'homozygous_label' => 'Sunburst (homozygot)',
            'display_order' => 18,
        ],
        [
            'name' => 'Jaguar',
            'slug' => 'jaguar',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Jaguar reduziert die Musterung stark und erzeugt goldene Rückenlinien. Die Superform ist nahezu zeichnungslos.',
            'normal_label' => 'Wildtyp',
            'heterozygous_label' => 'Jaguar',
            'homozygous_label' => 'Super Jaguar',
            'display_order' => 19,
        ],
        [
            'name' => 'Goldenrod',
            'slug' => 'goldenrod',
            'inheritance_mode' => 'dominant',
            'description' => 'Goldenrod ist eine liniengezüchtete Variante mit warmen Honigtönen und klaren Sattelflecken.',
            'normal_label' => 'Normal',
            'heterozygous_label' => 'Goldenrod',
            'homozygous_label' => 'Goldenrod (homozygot)',
            'display_order' => 20,
        ],
        [
            'name' => 'Frosted',
            'slug' => 'frosted',
            'inheritance_mode' => 'recessive',
            'description' => 'Frosted vereinigt Caramel und Evans Hypo Linien zu einem kühlen, nebeligen Erscheinungsbild.',
            'normal_label' => 'Normal',
            'heterozygous_label' => 'het Frosted',
            'homozygous_label' => 'Frosted',
            'display_order' => 21,
        ],
        [
            'name' => 'Pink Pastel',
            'slug' => 'pink-pastel',
            'inheritance_mode' => 'dominant',
            'description' => 'Line-bred Tiere mit lachsfarbenen Flanken und aufgehellter Kopfzeichnung.',
            'normal_label' => 'Normal',
            'heterozygous_label' => 'Pink Pastel',
            'homozygous_label' => 'Pink Pastel (homozygot)',
            'display_order' => 22,
        ],
        [
            'name' => 'Lightning Line',
            'slug' => 'lightning-line',
            'inheritance_mode' => 'dominant',
            'description' => 'Blitzhelle Tiere mit reduzierten dunklen Pigmenten und eisigen Highlights.',
            'normal_label' => 'Normal',
            'heterozygous_label' => 'Lightning Line',
            'homozygous_label' => 'Lightning Line (homozygot)',
            'display_order' => 23,
        ],
        [
            'name' => 'Red Albino',
            'slug' => 'red-albino',
            'inheritance_mode' => 'recessive',
            'description' => 'Selektierte Albino-Linie mit satten roten Pigmenten und rubinfarbenen Augen.',
            'normal_label' => 'Normal',
            'heterozygous_label' => 'het Red Albino',
            'homozygous_label' => 'Red Albino',
            'display_order' => 24,
        ]
    ];
    ...
    $referenceMorphs = [
        ...
        [
            ...
            'slug' => 'swiss-chocolat',
            'inheritance_mode' => 'recessive',
            'description' => 'Swiss Chocolate ist ein hypermelanistischer Morph. Die Hakennasennattern haben einen dunklen, fast einfarbigen Kopf.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
        ...
        [
            'name' => 'Swiss Chocolate Anaconda',
            'slug' => 'swiss-chocolate-anaconda',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Hier trifft der hypermelanistische Swiss-Chocolate-Look auf die Musterreduktion des Anaconda-Gens.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Swiss Chocolate Anaconda',
            'display_order' => 72,
            'is_reference' => true,
        ],
        [
            'name' => 'Swiss Chocolate Superconda',
            'slug' => 'swiss-chocolate-superconda',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Die Superform kombiniert zwei Anaconda-Allele mit dem Swiss-Chocolate-Hypermelanismus für nahezu einfarbig dunkle Tiere.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Swiss Chocolate Superconda',
            'display_order' => 73,
            'is_reference' => true,
        ],
        [
        ...
        [
            'name' => 'Swiss Chocolate Arctic Superconda',
            'slug' => 'swiss-chocolate-arctic-superconda',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Dreifachkombination aus Swiss Chocolate, Arctic und Superconda – extrem dunkle Patternless-Tiere mit eisigen Seitenschuppen.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Swiss Chocolate Arctic Superconda',
            'display_order' => 85,
            'is_reference' => true,
        ],
        [
            ...
            'slug' => 'shadow-arctic',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Shadow x Arctic erzeugt rauchige Tiere mit kalter Grundfarbe und reduziertem Kopfmuster.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Shadow Arctic',
            'display_order' => 86,
            'is_reference' => true,
        ],
        [
            'name' => 'Jaguar Superconda',
            'slug' => 'jaguar-superconda',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Die Superform des Jaguar-Morphs kombiniert mit zwei Anaconda-Genen für nahezu zeichnungslose Tiere.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Jaguar Superconda',
            'display_order' => 87,
            'is_reference' => true,
        ],
        [
            'name' => 'Goldenrod Arctic',
            'slug' => 'goldenrod-arctic',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Goldenrod verleiht warme Farbtöne, Arctic sorgt für hohe Kontraste – beliebte Double-Het-Kombi.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Goldenrod Arctic',
            'display_order' => 88,
            'is_reference' => true,
        ],
        [
            ...
            'slug' => 'frosted-toffee',
            'inheritance_mode' => 'recessive',
            'description' => 'Doppelt rezessive Kombination mit cremig-weißer Grundfarbe und karamellfarbenen Sattelflecken.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Frosted Toffee',
            'display_order' => 89,
            'is_reference' => true,
        ],
        [
            'name' => 'Red Albino Anaconda',
            'slug' => 'red-albino-anaconda',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Rot gesättigte Albinos mit Anaconda-Musterreduktion – hohe Nachfrage in der Designerzucht.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Red Albino Anaconda',
            'display_order' => 90,
            'is_reference' => true,
        ],
        [
            'name' => 'Pink Pastel Conda',
            'slug' => 'pink-pastel-conda',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Selektionslinie mit Anaconda: rosige Flanken treffen auf reduzierte Zeichnung.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Pink Pastel Conda',
            'display_order' => 91,
            'is_reference' => true,
        ],
        [
            'name' => 'Lightning Arctic',
            'slug' => 'lightning-arctic',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Arctic Kontraste verstärken die eisigen Töne der Lightning-Linie für nahezu monochrome Tiere.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Lightning Arctic',
            'display_order' => 92,
            'is_reference' => true,
        ]
    ];
    ...
    $hognoseImages = [
        'sunrise' => 'https://images.unsplash.com/photo-1610970878458-1c0d565c4ce6?auto=format&fit=crop&w=1200&q=80',
        'sand' => 'https://images.unsplash.com/photo-1542401886-65d6c61db217?auto=format&fit=crop&w=1200&q=80',
        'macro' => 'https://images.unsplash.com/photo-1581888227599-779811939961?auto=format&fit=crop&w=1200&q=80',
        'focus' => 'https://images.unsplash.com/photo-1571832261077-7bf11ef1a978?auto=format&fit=crop&w=1200&q=80',
    ];
    ...
    if ($pogona) {
        $pogonaId = (int)$pogona['id'];
        $currentDescription = trim((string)($pogona['description'] ?? ''));
        if ($currentDescription === '' || stripos($currentDescription, 'Bartagame') === false) {
            update_genetic_species($pdo, $pogonaId, [
                'name' => $pogona['name'],
                'slug' => $pogona['slug'],
                'scientific_name' => $pogona['scientific_name'],
                'description' => $pogonaDescription,
            ]);
        }
    } else {
        $pogonaId = create_genetic_species($pdo, [
    ...
    $pogonaGenes = [
        [
            ...
            'slug' => 'pogona-hypo',
            'inheritance_mode' => 'recessive',
            'description' => 'Hypomelanistische Bartagamen zeigen deutlich reduzierte Schwarztöne und wirken insgesamt heller und pastelliger.',
            'normal_label' => 'Normal',
        ...
        [
            'name' => 'Translucent',
            'slug' => 'pogona-translucent',
            'inheritance_mode' => 'recessive',
            'description' => 'Translucente Jungtiere besitzen tiefblaue Augen und halbtransparente Schuppen, die mit dem Alter nachdunkeln.',
            'normal_label' => 'Normal',
        ...
        [
            'name' => 'Leatherback',
            'slug' => 'pogona-leatherback',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Leatherbacks besitzen deutlich reduzierte Rückenkiele und dadurch ein glatteres, farbintensiveres Schuppenkleid.',
            'normal_label' => 'Normal',
        ...
        [
            ...
            'slug' => 'pogona-dunner',
            'inheritance_mode' => 'dominant',
            'description' => 'Dunner-Bartagamen zeigen verwirbelte Muster und nach hinten gerichtete Schuppen, die in der Superform besonders ausgeprägt sind.',
            'normal_label' => 'Normal',
        ...
        [
            'name' => 'Witblits',
            'slug' => 'pogona-witblits',
            'inheritance_mode' => 'recessive',
            'description' => 'Witblits entfernen nahezu sämtliche Musterungen und erzeugen ein homogenes, sehr helles Erscheinungsbild.',
            'normal_label' => 'Normal',
        ...
        [
            ...
            'slug' => 'pogona-zero',
            'inheritance_mode' => 'recessive',
            'description' => 'Zero Bartagamen sind komplett patternless und besitzen kühle graue bis silberne Farbtöne.',
            'normal_label' => 'Normal',
    ...
    $pogonaReferences = [
        [
            'name' => 'Hypo Translucent',
            'slug' => 'pogona-hypo-translucent',
            'inheritance_mode' => 'recessive',
            'description' => 'Kombination aus Hypomelanismus und Translucent: sehr klare Pastelltöne mit transparenten Schuppen und blauen Augen.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Hypo Translucent',
            'display_order' => 10,
            'is_reference' => true,
        ],
        [
            'name' => 'Hypo Leatherback',
            'slug' => 'pogona-hypo-leatherback',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Die Kombination liefert extrem glatte, farbintensive Tiere mit reduzierten Dunkelpigmenten.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Hypo Leatherback',
            'display_order' => 11,
            'is_reference' => true,
        ],
        [
            'name' => 'Translucent Leatherback',
            'slug' => 'pogona-translucent-leatherback',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Translucente Leatherbacks zeigen ein seidiges Schuppenkleid mit leicht transparenten Schuppenrändern.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Translucent Leatherback',
            'display_order' => 12,
            'is_reference' => true,
        ],
        [
            'name' => 'Hypo Translucent Leatherback',
            'slug' => 'pogona-hypo-translucent-leatherback',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Der Klassiker unter Designer-Bartagamen: seidenweiche Haut, transparente Schuppen und kaum Schwarztöne.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Hypo Translucent Leatherback',
            'display_order' => 13,
            'is_reference' => true,
        ],
        [
            'name' => 'Zero Translucent',
            'slug' => 'pogona-zero-translucent',
            'inheritance_mode' => 'recessive',
            'description' => 'Translucente Zeros wirken wie silbern schimmernde Glas-Tiere ohne jegliche Musterung.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Zero Translucent',
            'display_order' => 14,
            'is_reference' => true,
        ],
        [
            'name' => 'Witblits Translucent',
            'slug' => 'pogona-witblits-translucent',
            'inheritance_mode' => 'recessive',
            'description' => 'Fast transparente Pastelltöne durch das Zusammenspiel aus Witblits und Translucent.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Witblits Translucent',
            'display_order' => 15,
            'is_reference' => true,
        ],
        [
            'name' => 'Zero Witblits (Wero)',
            'slug' => 'pogona-zero-witblits-wero',
            'inheritance_mode' => 'recessive',
            'description' => 'Wero ist die doppelt rezessive Kombination aus Zero und Witblits – komplett patternless und äußerst kontrastarm.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Zero Witblits (Wero)',
            'display_order' => 16,
            'is_reference' => true,
        ],
        [
            'name' => 'Wero Translucent',
            'slug' => 'pogona-wero-translucent',
            'inheritance_mode' => 'recessive',
            'description' => 'Verstärkt den Glas-Effekt des Wero Morphs noch weiter und sorgt für leicht bläuliche Schimmer.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Wero Translucent',
            'display_order' => 17,
            'is_reference' => true,
        ],
        [
            'name' => 'Hypo Wero',
            'slug' => 'pogona-hypo-wero',
            'inheritance_mode' => 'recessive',
            'description' => 'Die Kombination reduziert sämtliche dunklen Pigmente – fast weiße Tiere mit subtilen Pastellverläufen.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Hypo Wero',
            'display_order' => 18,
            'is_reference' => true,
        ],
        [
            'name' => 'Hypo Zero',
            'slug' => 'pogona-hypo-zero',
            'inheritance_mode' => 'recessive',
            'description' => 'Komplett patternless Bartagamen ohne Schwarztöne – beliebt als „Snow Zero“ in der Szene.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Hypo Zero',
            'display_order' => 19,
            'is_reference' => true,
        ],
        [
            'name' => 'Hypo Witblits',
            'slug' => 'pogona-hypo-witblits',
            'inheritance_mode' => 'recessive',
            'description' => 'Sehr helle Tiere ohne Musterung, die rosa Pastelltöne zeigen.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Hypo Witblits',
            'display_order' => 20,
            'is_reference' => true,
        ],
        [
            'name' => 'Leatherback Zero',
            'slug' => 'pogona-leatherback-zero',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Völlig glatte, patternless Tiere mit seidig wirkender Oberfläche.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Leatherback Zero',
            'display_order' => 21,
            'is_reference' => true,
        ],
        [
            'name' => 'Leatherback Witblits',
            'slug' => 'pogona-leatherback-witblits',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Glatte Pastelltöne ohne Muster – ideal als Grundlage für weitere Designerprojekte.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Leatherback Witblits',
            'display_order' => 22,
            'is_reference' => true,
        ],
        [
            'name' => 'Dunner Leatherback',
            'slug' => 'pogona-dunner-leatherback',
            'inheritance_mode' => 'incomplete_dominant',
            'description' => 'Verwirbelte Muster treffen auf ein seidiges Schuppenkleid – eine beliebte Designer-Kombi.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Dunner Leatherback',
            'display_order' => 23,
            'is_reference' => true,
        ],
        [
            'name' => 'Dunner Hypo',
            'slug' => 'pogona-dunner-hypo',
            'inheritance_mode' => 'dominant',
            'description' => 'Dunner-Muster mit reduzierter Melanindichte – besonders kontrastreiche Tiere.',
            'normal_label' => 'Nicht kombiniert',
            'heterozygous_label' => 'Teil-Kombination',
            'homozygous_label' => 'Komplett: Dunner Hypo',
            'display_order' => 24,
            'is_reference' => true,
        ],
    ];
    ...
    $pogonaImages = [
        'sunset' => 'https://images.unsplash.com/photo-1501706362039-c6e08e3f663e?auto=format&fit=crop&w=1200&q=80',
        'forest' => 'https://images.unsplash.com/photo-1526318472351-c75fcf070305?auto=format&fit=crop&w=1200&q=80',
        'portrait' => 'https://images.unsplash.com/photo-1587731549560-5f72f66445b1?auto=format&fit=crop&w=1200&q=80',
    ];
