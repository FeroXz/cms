...
function ensure_default_settings(PDO $pdo): void
{
    $defaults = [
        ...
        'footer_text' => '© ' . date('Y') . ' FeroxZ CMS — Version 5.6',
    ...
    if ($currentFooter !== '') {
        $updatedFooter = preg_replace('/Version\s+\d+\.\d+/', 'Version 5.6', $currentFooter);
    ...
    if ($storedVersion !== '5.6') {
        set_setting($pdo, 'cms_version', '5.6');
    }
}
